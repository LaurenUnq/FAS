...
def mesurerFC(pin,time): #pin est le pin sur lequel est le capteur, time et le temps pendant lequel on prend la fréquence (ex:20sec)
	pinMode(sens,"INPUT") #plus le temps time est grand plus ce sera précis
	current = time.time()
	prec = 0
	count = 0
	sec = current
